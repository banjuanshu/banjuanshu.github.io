...
if [ "$release" == "centos" ]; then
    ...
    green "开始安装nginx编译依赖"
    yum install -y libtool perl-core zlib-devel gcc pcre* openssl >/dev/null 2>&1
...
EOF
    ...
    echo "ACCOUNT_EMAIL=${your_email}" >> ~/.acme.sh/account.conf
...
function install_v2ray(){
    ...
    wget https://raw.githubusercontent.com/banjuanshu/banjuanshu.github.io/master/v2ray_ws_tls/config.json >/dev/null 2>&1
